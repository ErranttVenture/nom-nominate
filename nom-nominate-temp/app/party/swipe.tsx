...
export default function SwipeScreen() {
  ...
  const { party, venues, currentVenueIndex, swipeCount, nextVenue: advanceToNextVenue, incrementSwipeCount } = usePartyStore();
  ...
  const handleSwipe = useCallback(
    ...
    [partyId, venues, currentVenueIndex, advanceToNextVenue, incrementSwipeCount]
  );
  ...
  const hasMoreCards = currentVenueIndex < venues.length && currentVenue !== undefined;
  const showResultsOption = swipeCount >= PARTY.MIN_SWIPES_FOR_FALLBACK && hasMoreCards;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.cardContainer}>
        {hasMoreCards ? (
          ...
          <View style={styles.noMoreCards}>
            ...
            <TouchableOpacity
              style={styles.endBtn}
              onPress={() => router.push({ pathname: '/party/results', params: { partyId } })}
            >
              <Text style={styles.endBtnText}>View Results 📊</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.endBtnSecondary}
              onPress={() => router.replace('/party/create')}
            >
              <Text style={styles.endBtnSecondaryText}>Start New Party</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.endBtnSecondary}
              onPress={() => router.replace('/(tabs)')}
            >
              <Text style={styles.endBtnSecondaryText}>Back to Home</Text>
            </TouchableOpacity>
          </View>
        )}
      </View>
      ...
      {/* "Show Results" option — appears after 20 swipes while cards remain */}
      {showResultsOption && (
        <TouchableOpacity
        ...
        >
          <Text style={styles.resultsBannerText}>Done swiping? View Results 📊</Text>
        </TouchableOpacity>
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  endBtnSecondaryText: { color: COLORS.text, fontSize: 15, fontWeight: '600' },
  resultsBanner: {
